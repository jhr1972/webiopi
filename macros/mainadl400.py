...
def read_double_bulk (client, lregs ):
    ...
    try:
        resp = client.read_holding_registers(lregs[0][1],count=bulklen, unit=13)
...
def main() :
    regs = [
        # Symbol    Reg#  Format
         ( 'Va'     ,0x6022, '%6.1f',1 ), # Voltage Phase A [V]
    ...
    double_regs = [
        # Symbol    Reg#  Format
         ( 'Pa_compl'     ,0xA, '%6.1f',1 ), # Active Power with complement
         ( 'Pb_compl'     ,0xC, '%6.1f',1 ) # Active Power with complement
