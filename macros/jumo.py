...
regs = [
   ( 'Test'     ,0x1F, '%6.2f',2) # Current Phase A[A]
...
cl = pymodbus.client.ModbusSerialClient( port='/dev/ttyUSB0', baudrate=9600, parity='N',stopbits=1, timeout=3)
...
def read_double_bulk (client, lregs ):
    ...
    try:
        resp = client.read_holding_registers(lregs[0][1],count=bulklen, slave=2)
